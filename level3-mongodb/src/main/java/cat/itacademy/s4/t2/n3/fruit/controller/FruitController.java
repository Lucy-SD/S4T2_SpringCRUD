...
import cat.itacademy.s4.t2.n3.fruit.service.FruitService;
...
public class FruitController {
    ...
    @GetMapping("/{id}")
    public FruitResponse getFruitById(@PathVariable String id) {
        Fruit fruit = fruitService.getFruitById(id);
        return FruitResponse.from(fruit);
    }

    @PutMapping("/{id}")
    public FruitResponse updateFruit(@PathVariable String id, @Valid @RequestBody CreateFruitRequest fruitRequest) {
    ...
    public void deleteFruitById(@PathVariable String id) {
